...
3. **▶ 흐름 검증** — **(자동)** Playwright로 `00-flow.md` 기준 흐름 테스트를 green까지 루프(A→B 이동 / 막다른 길 / 범위 안 도달 / 일괄액션 결과 도달) → **(사람)** "맞는 흐름인가" 판단. 수정은 **색칠 전에** 끝낸다.
4. **디자인 시스템** — 흐름 확정 후 `tokens.css` + 기본 컴포넌트 확정.
5. **하이파이 (hi-fi)** — 토큰 입힌 실제 화면. 링크 구조 유지 + 전환·애니메이션·상태(로딩/빈/에러).
6. **▶ 인터랙션 검증** — **(자동)** Playwright로 상태 전이·버튼 동작 green까지 → **(사람)** 전환·애니메이션 *느낌* 판단.
7. **handoff** — 대상 프로젝트의 CLAUDE.md를 읽고 그 컨벤션으로 변환해 넘긴다.

> **흐름 검증은 lo-fi(3), 느낌 검증은 hi-fi(6).** 다른 테스트다. 섞지 않는다.
> Playwright는 *내비게이션·구조·상태*만 검증한다. 비주얼·느낌 판단은 사람 몫.

### 묶음(청크) 진행 순서 — 큰 PRD

화면이 많으면 묶음으로 나누되, **파이프라인은 "단계(세로)"로 자르지 "묶음(가로)"으로 자르지 않는다.**

- ✅ 한 **단계**를 **모든 묶음**에 끝내고 게이트 통과 후 다음 단계로.
- ❌ 한 **묶음**을 와이어→토큰→하이파이 끝까지 내리기 — 금지.

```
와이어: 전 묶음 → 흐름 검증: 전 묶음 통과 → tokens.css(1회) → 하이파이: 묶음별 → 인터랙션 검증 → handoff
```

**왜:** `tokens.css`는 제품 전체가 공유하는 토대라 모든 묶음 구조가 확정된 뒤 한 번에 만든다. 1묶음만 보고 만들면 뒤 묶음에서 계속 땜질하게 된다.
