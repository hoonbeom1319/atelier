...
- **무인 변형 `/forge`** — 위 흐름을 **사람 OK 게이트 없이** 끝까지 자동으로 돌린다. *자동 게이트(`lint-prd`·Playwright 1·2층·a11y)는 그대로 유지*하고, *사람 OK·3층 판단*만 **시작 1회 인테이크 + 가정 원장**으로 대체해 **hi-fi만 전달**한다(5.5·6 느낌·7 handoff는 사람이 트리거). 절차 원본은 `forge/SKILL.md`.
