...
4. **디자인 시스템** — 흐름 확정 후 `tokens.css` + 기본 컴포넌트 확정. **토큰 확정 즉시 대비비(a11y)를 자동 검사**(`scripts/lib/a11y.js`) — 미달이면 색 입히기 전에 토큰을 고친다.
...
7. **handoff** — **chosen final**을 *패키지 + 인계 프롬프트*로 묶는다. `handoff/`에 비주얼 문서(`index.html`)·토큰 매핑표·컴포넌트 인벤토리 + ★**인계 프롬프트 `hand-off.md`**(받는 쪽 Claude Code에 붙여넣는 브리프 — 참고 파일·지킬 계약·만들 순서 명시). **묶은 뒤 `node scripts/lint-handoff.js <name>`로 완결성 자동 검사** — green 없이 "완료" 선언 금지. **atelier는 대상 코드를 직접 짜지 않는다** — 이 프롬프트를 넘기면 대상 Claude가 자기 컨벤션으로 구현(특정 프레임워크에 안 묶임). 직접 호출 가능(재-handoff·다중 대상).

> **검증은 3층:** 내비게이션(자동) + 기능 완결성(자동) + 느낌·판단(사람). "자동은 내비게이션만"이 아니다 — 화면이 책임지는 **동작·상태 변화도 자동이 green까지** 본다. 토큰 대비비(a11y)·handoff 완결성도 자동이 본다. 비주얼·느낌·"옳은 흐름인가"만 사람 몫.
...
- **② 단계 *안에선* 묶음마다 게이트** — 와이어 단계에서 전 화면을 한꺼번에 쏟고 "다 됐어, 전체검증해줘"로 가지 **않는다.** 묶음1 와이어 → 자동 green + 사람 OK → 묶음2 → … 끊어서 내린다. 묶음1 구조가 검증된 뒤 묶음2가 물려받아야 품질이 오른다. (①·②는 충돌이 아니라 다른 높이의 규칙.)
