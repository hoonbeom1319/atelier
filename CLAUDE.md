...
- **무인 변형 `/forge`** — 위 흐름을 **사람 OK 게이트 없이** 끝까지 자동으로 돌린다. *자동 게이트(`lint-prd`·Playwright 1·2층·a11y)는 그대로 유지*하고, *사람 OK·3층 판단*은 **시작 1회 인테이크 + 가정 원장 + 전문가 에이전트(시장조사·트렌드, 웹검색)** 로 대체해 **인계 패키지(hand-off)까지 전달**한다. 시장조사가 plan 답을 근거화(`plan-decisions.md`), 트렌드 전문가가 hi-fi를 감정해 미달 시 자동 리디자인 1회(`design-critique.md`), 7 handoff는 자동(`lint-handoff` 게이트)으로 `hand-off.md`까지. 추가 발산·6 느낌검수만 사람이 트리거. 절차 원본은 `forge/SKILL.md`.
