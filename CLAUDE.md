...
│   ├── prd-critic.md               #   PRD 독립 비평가(8차원 실질 감정 — 범위 부풀림·골격 빈약 양방향, prd-review.md 작성) — plan §C / forge §B
│   ├── design-verifier.md          #   하이파이 독립 검증자(render-check 7항목, 렌더 PNG 기반, design-verify.md 작성) — design §C step 5
│   ├── screen-builder.md           #   화면 1개 빌더(하이파이/리디자인, anti-wireframe 바) — 빌드 fan-out 단위
│   ├── screen-shooter.md           #   렌더러(Bash+Playwright) — hi-fi를 헤드리스 렌더해 PNG로(검증·트렌드가 픽셀을 봄) — forge §C-5 Shoot
│   └── design-trend-expert.md      #   비주얼 레퍼런스 브리프(빌드 전) + 트렌드 감정(빌드 후, 렌더 기반 0~100) — forge §C-5·6
...
│   ├── shoot.js                    # hi-fi 헤드리스 렌더 → PNG(_shots/) — 검증·트렌드 에이전트가 픽셀을 보게(D1)
│   ├── lint-prd.js                 # PRD 형식 검사(plan→design 게이트: 11섹션·깊이)
│   ├── lint-prd-review.js          # PRD 실질 비평 게이트(prd-review.md: 별도 Agent가 8차원 비평했나)
│   ├── lint-verify.js              # 하이파이 독립 검증 게이트(design-verify.md: 별도 Agent가 render-check 했나)
│   ├── pack-handoff.js             # handoff self-contained 패키저(PRD·흐름·화면·토큰을 handoff/에 복사 + ../→./ 재작성)
│   ├── lint-handoff.js             # handoff 완결성+self-contained 자동 검사(design→dev 게이트)
...
└── projects/
    ...
    │   ├── prd-review.md           # PRD 독립 비평 (prd-critic 8차원 판정 — lint-prd-review가 검사)
    ...
    │   └── handoff/                # ★ self-contained 인계 패키지 (이 폴더만 떼서 넘겨도 열림)
    │       ├── hand-off.md         #   받는 쪽에 붙여넣는 브리프 (경로 전부 ./ 상대)
    │       ├── index.html · token-mapping.md · component-inventory.md
    │       └── PRD.md · 00-flow.md · screens/ · foundation/ · components/ · _shots/  # pack-handoff.js가 복사
    ├── my-second-app/
    └── my-third-app/
```
...
- **검증은 3층이고, 자동이 2층까지 본다.** 내비게이션(자동)+기능 완결성(자동, 죽은 컨트롤 0)+느낌·판단(사람). PRD 실질 비평(`lint-prd-review` — 별도 Agent가 8차원 비평했나, 부풀림+골격 빈약 양방향)·토큰 대비비(a11y)·하이파이 독립 검증(`lint-verify` — 별도 Agent가 render-check 했나)·handoff 완결성도 자동(스크립트)이 본다 — 사람은 비주얼·느낌·"옳은 흐름인가"만. 자동 게이트는 3·6단계(Playwright)와 5단계(`lint-verify`)·경계(`lint-prd`+`lint-prd-review`/`lint-handoff`)에 있다.
...
7. **wireframey** — (하이파이 전용) 렌더가 "와이어 + 색" 수준인가(깨지지도 빈약하지도 않은데 깊이·여백 리듬·타이포 위계·마이크로 인터랙션이 없어 hi-fi가 아님). thin/bad가 못 잡는 *비주얼 충실도* — **렌더 스크린샷(픽셀)으로 판정**(소스만으론 못 잡음).

---

## handoff

- **handoff는 "코드 변환"이 아니라 "인계 프롬프트 전달"이다.** chosen final을 `handoff/` 패키지(비주얼 문서·토큰 매핑표·컴포넌트 인벤토리)로 묶고, **`hand-off.md`** = 받는 쪽 Claude Code에 붙여넣는 브리프(참고 파일·지킬 계약·만들 순서)를 만든다. 대상 프로젝트의 Claude가 그걸 보고 **자기 컨벤션으로** 짠다. **atelier는 대상 코드를 직접 짜지 않는다 — 특정 프레임워크·아키텍처에 묶이지 않는다.**
- **★ `handoff/`는 self-contained — 그 폴더 *하나만* 떼서 대상 프로젝트에 넘겨도 전부 열려야 한다.** 인계에 필요한 소스(`PRD.md`(DB 설계 근거)·`00-flow.md`·chosen 화면·`foundation/`·`components/`·`_shots/`)를 `handoff/` 안에 **같은 상대 레이아웃으로 복사**하고, 패키지 문서의 경로는 전부 **`handoff/` 루트 기준 `./` 상대**로 쓴다(`../`·`projects/<name>/`로 패키지 밖을 가리키면 안 됨 — `<대상>/CLAUDE.md` 등 받는 쪽 저장소 파일만 예외). 이 복사·경로 재작성은 **`scripts/pack-handoff.js`** 가 하고, **`scripts/lint-handoff.js`** 가 self-contained 위반(필요 소스 미복사·handoff/ 밖 참조)을 에러로 막는다. **PRD를 참조 경로로만 두고 빼지 않는다 — 빼면 받는 쪽이 데이터 모델(DB)을 추측하게 된다.**
