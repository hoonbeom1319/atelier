...
- **검증은 3층이고, 자동이 2층까지 본다.** 내비게이션(자동)+기능 완결성(자동, 죽은 컨트롤 0)+느낌·판단(사람). 토큰 대비비(a11y)·하이파이 독립 검증(`lint-verify` — 별도 Agent가 render-check 했나)·handoff 완결성도 자동(스크립트)이 본다 — 사람은 비주얼·느낌·"옳은 흐름인가"만. 자동 게이트는 3·6단계(Playwright)와 5단계(`lint-verify`)·경계(`lint-prd`/`lint-handoff`)에 있다.
