...
- PRD를 다 쓰면 **`node scripts/lint-prd.js <name>`로 완결성을 자동 검사**한다(plan→design 게이트) — 11개 섹션이 다 있나·§5 관계표기·§8 다크모드·뷰포트 같은 깊이를 본다. green 없이 `/design`으로 넘기지 않는다(design→dev의 `lint-handoff`와 대칭).
...
```
PRD ─▶ ① 인테이크 ─▶ ② 와이어(클릭됨) ─▶ ③ ▶흐름·기능 검증 ─▶ ④ 디자인 시스템
                                                                       │
⑦ handoff ◀─ ⑥ ▶인터랙션 검증 ◀─ (⑤.5 리디자인·선택) ◀─ ⑤ 하이파이 ◀─┘
```
...
- **검증은 3층** — 내비게이션(자동)·화면이 책임지는 **기능 완결성**(자동, 죽은 컨트롤 0)·느낌과 "옳은 흐름인가"(사람). 자동 검증은 ③·⑥ 두 군데서 green까지 루프(Playwright). 반복 로직은 `scripts/lib/`에 골격으로 박혀 있어 매 프로젝트 재발명하지 않는다. **a11y도 자동이 본다** — 토큰 대비비(값 차원) + 렌더 화면 axe 검사(대비·역할·포커스, `scripts/lib/a11y.js`).
- **⑤.5 리디자인은 선택** — hi-fi가 "와이어 + 색"에 머물러 비주얼 도약이 필요할 때만. 흐름·기능은 지킨 채 아트디렉션을 재해석해 후보(`screens-<variant>/`)를 만들고, 각 후보는 **기능 spec을 그대로 통과**해야 한다(가드레일). 사용자가 **chosen final**을 고른다.
- **큰 PRD는 묶음(청크)으로** — 화면이 많으면 단계 안에서 묶음마다 게이트를 건다(묶음1 와이어 검증·OK → 묶음2 → …). 단, `tokens.css`는 전체 공유 토대라 모든 묶음 구조가 확정된 뒤 한 번에 만든다.
...
> 패키지를 묶은 뒤 `node scripts/lint-handoff.js <name>`로 **완결성을 자동 검사**한다 — chosen 화면이 다 실렸나·semantic 토큰이 매핑표에 다 있나·컴포넌트가 인벤토리에 다 있나·placeholder가 안 남았나. green 없이 "완료" 없다.
