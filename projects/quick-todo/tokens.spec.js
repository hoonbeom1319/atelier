// quick-todo — C-4 게이트: 토큰 대비비 WCAG AA (라이트 + 다크 양 모드).
// a11y.js의 resolveTokens는 파일 내 마지막 정의를 채택하므로(다크가 라이트를 덮음),
// 모드별로 CSS를 분리해 *양쪽 다* 검사한다 — 한 모드만 보고 green 되는 구멍 방지.
//   실행: node scripts/test-project.js quick-todo projects/quick-todo/tokens.spec.js
const fs = require('fs');
...
for (const [mode, css] of [['light', lightCss], ['dark', darkCss]]) {
  test(`자동 a11y: 토큰 대비비 WCAG AA — ${mode} 모드`, () => {
    ...
    expect(evaluated, `${mode}: 검사된 쌍이 0개 — semantic 토큰명 확인`).toBeGreaterThanOrEqual(4);
